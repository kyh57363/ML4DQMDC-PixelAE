...
def evaluate_autoencoders_combined(logprob_good, logprob_bad, fmBiasFactor, wpBiasFactor):
    ...
    badLabels = labels[len(logprob_good):]
    logprob_good_temp = np.concatenate((logprob_good, logprob_bad[badLabels == 0]))
    logprob_bad_temp = logprob_bad[badLabels == 1] 
 
    separable = logprob_bad_temp[logprob_bad_temp < min(logprob_good_temp)]
    sepPercB = len(separable) / len(logprob_bad_temp)
    separable = logprob_good_temp[logprob_good_temp > max(logprob_bad_temp)]
    sepPercG = len(separable) / len(logprob_good_temp)
